...
func (ml *messagesList) drawAuthor(w io.Writer, message discord.Message) {
	...
	if message.GuildID.IsValid() && !message.WebhookID.IsValid() {
		...
		if err != nil {
			// Member not loaded yet - this is normal, just use default name
			slog.Debug("member not in state, using default name", "guild_id", message.GuildID, "member_id", message.Author.ID)
		} else {
			if member.Nick != "" {
				name = member.Nick
			}
...
func (ml *messagesList) reply(mention bool) {
	...
	if msg.GuildID.IsValid() {
		...
		if err != nil {
			// Member not loaded yet - this is normal, just use default name
			slog.Debug("member not in state, using default name", "guild_id", msg.GuildID, "member_id", msg.Author.ID)
		} else {
			if member.Nick != "" {
				name = member.Nick
			}
